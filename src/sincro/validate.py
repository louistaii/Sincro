...
def validate(inst: Instance, sub_dir: str | Path, scenario: str | None = None) -> dict:
    ...
    # Charge every activity's rate against its contract's final lateness.
    # Individual activity lateness remains a separate diagnostic, not the score.
    for aid, wks in weeks_of.items():
        if not wks:
            continue
        a = inst.activities[aid]
    ...
    if not violations:
        ...
        soft['formula_version'] = 'contract-final-delay-v5'
        soft['penalty_breakdown'] = {
            'contract_delay': round(weighted, 1) if scenario != 'B' else 0,
            'excess_access': 7 * excess if scenario != 'A' else 0,
            'eclo': 5 * eclo_nights if scenario != 'A' else 0,
        }
